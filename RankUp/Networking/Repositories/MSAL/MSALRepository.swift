...
final class MSALRepository: MSALService {
    ...
    private let kScopes: [String]       = ["https://graph.microsoft.com/Analytics.Read",
    ...
    func retrieveProfilePhoto(_ request: URLRequest?, completion: @escaping RetrieveProfilePhotoCompletion) {
        ...
        let task = URLSession.shared.downloadTask(with: request, completionHandler: { (location: URL?, response: URLResponse?, error: Error?) -> Void in
            switch BasicRequest.handleBasicResponse(with: try? Data(contentsOf: location!), response: response, error: error) {
